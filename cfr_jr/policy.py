...
def print_policy(policy, file=None, player=None):
	if player is None:
		for state, probs in zip(it.chain(*policy.states_per_player), policy.action_probability_array):
			if file is None:
				print(f'{state:6}   p={probs}')
			else:
				print(f'{state:6}   p={probs}', file=file)
	else:
		for state, probs in zip(policy.states_per_player[player], policy.action_probability_array):
			if file is None:
				print(f'{state:6}   p={probs}')
			else:
				print(f'{state:6}   p={probs}', file=file)
